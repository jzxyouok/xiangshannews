...
import com.zrok.xiangshannews.adapter.DragAdapter;
import com.zrok.xiangshannews.adapter.OtherAdapter;
...
import com.zrok.xiangshannews.manager.ChannelManage;
import com.zrok.xiangshannews.view.DragGrid;
import com.zrok.xiangshannews.view.OtherGridView;

public class ChinnelActivity extends Activity implements OnItemClickListener{
	/** 用户栏目的GRIDVIEW */
	...
	private void initData(){
		userChannelList = DBHelper.getInstence(getApplicationContext()).getUserChannels();
		otherChannelList = DBHelper.getInstence(getApplicationContext()).getOtherChannels();
                        ...
                        new Handler().postDelayed(new Runnable() {
                            ...
                            public void run() {
                                try {
                                    ...
                                    DBHelper.getInstence(getApplicationContext()).updateChannelForType(channel, 0);
                    ...
                    new Handler().postDelayed(new Runnable() {
                        ...
                        public void run() {
                            try {
                                ...
                                DBHelper.getInstence(getApplicationContext()).updateChannelForType(channel, 1);
                            } catch (Exception localException) {
                            }
                        }
                    }, 50L);
    ...
    /** 退出时候保存选择后数据库的设置 */
    void saveChannel() {
    	DBHelper.getInstence(getApplicationContext()).clearAllChannel();
    	ChannelManage.getManage(getApplicationContext()).saveUserChannel(userAdapter.getChannnelLst());
    	ChannelManage.getManage(getApplicationContext()).saveOtherChannel(otherAdapter.getChannnelLst());
    }
    ...
    public void finish() {
        ...
        if (userAdapter.isListChanged()) {
        	AppApplication.getApp().isChange = true;
        	saveChannel();
        }
    }
